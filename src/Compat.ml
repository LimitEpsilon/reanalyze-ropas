...
let getStringValue const =
  match const with
  | ((Parsetree.Pconst_string (s, _, _)) [@if ocaml_version >= (4, 11, 0)]) -> s
  | ((Parsetree.Pconst_string (s, _)) [@if ocaml_version < (4, 11, 0)]) -> s
  | _ -> assert false

let getConstString const =
  match const with
  | ((Asttypes.Const_string (s, _, _)) [@if ocaml_version >= (4, 11, 0)]) -> s
  | ((Asttypes.Const_string (s, _)) [@if ocaml_version < (4, 11, 0)]) -> s
  | _ -> assert false
...
let unboxPatCstrName pat =
  ...
  | Typedtree.Tpat_value v -> (
    ...
    [@if ocaml_version >= (4, 13, 0)]) ->
      Some cstr_name
    | ((Tpat_construct (_, {cstr_name}, _)) [@if ocaml_version < (4, 13, 0)]) ->
      Some cstr_name
    | _ -> None)
  | _ -> None
...
let unboxPatCstrTxt pat =
  ...
  [@if ocaml_version >= (4, 13, 0)]) ->
    txt
  | ((Typedtree.Tpat_construct ({txt}, _, _)) [@if ocaml_version < (4, 13, 0)])
    ->
    txt
  | _ -> assert false
...
let getSigType si =
  match si with
  | ((Types.Sig_type (id, t, _, _)) [@if ocaml_version >= (4, 08, 0)]) -> (id, t)
  | ((Types.Sig_type (id, t, _)) [@if ocaml_version < (4, 08, 0)]) -> (id, t)
  | _ -> assert false

let getTSubst td =
  match td with
  | ((Types.Tsubst (t, _)) [@if ocaml_version >= (4, 13, 0)]) -> t
  | ((Types.Tsubst t) [@if ocaml_version < (4, 13, 0)]) -> t
  | _ -> assert false

let getTypeVariant (tk : ('a, 'b) type_kind) =
  match tk with
  | ((Type_variant (l, _)) [@if ocaml_version >= (4, 13, 0)]) -> l
  | ((Type_variant l) [@if ocaml_version < (4, 13, 0)]) -> l
  | _ -> assert false
...
let texpMatchGetExceptions desc =
  match desc with
  | ((Typedtree.Texp_match (_, cases, _)) [@if ocaml_version >= (4, 08, 0)]) ->
    cases
...
let tstrExceptionGet (x : Typedtree.structure_item_desc) =
  ...
  | ((Tstr_exception {ext_id; ext_loc}) [@if ocaml_version < (4, 08, 0)]) ->
    Some (ext_id, ext_loc)
  | _ -> None
